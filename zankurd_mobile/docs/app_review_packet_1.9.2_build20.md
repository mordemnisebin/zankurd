# ZanKurd iOS 1.9.2 (20) — App Review Packet

Bu paket, mevcut kaynak koddan üretilen 1.9.2 (20) release adayı içindir.
...
7. Open the optional subscription screen. The shortest entry point is the
...
Metadata düzeltmesi build 18'de yapıldı. Build 19, ret mektubunun kaynak
listesindeki **uygulama içi** dört koşulu ayrıca sıkılaştırır: build 18
gönderilmeden önce yapılan taramada o listenin ikisi eksik çıktı.

Bu gönderimde kapatılanlar:

- **App Description**'a Apple standart EULA bağlantısı ve gizlilik
  politikası bağlantısı eklendi (build 18).
- **App Review Notes**'a aynı bilgi eklendi (Apple gelecekteki gönderimler
  için bunu açıkça istedi) (build 18).
- **App Privacy**'deki "Coarse Location" beyanı kaldırıldı. Uygulama konum
  ...
  bağımlılığı yok. Beyan binary ile çelişiyordu (build 18).
- **Aboneliğin adı satın alma ekranına yazıldı** (build 19). Ekranın tepesinde
  "Premium" yazıyordu; "Premium" bir özellik adıdır, App Store Connect'teki
  ürün adı değil. Başlık artık `AppConfig.subscriptionDisplayName` =
  "ZanKurd Pro" — mağazadaki abonelik grubunun adı. Kart başlıklarıyla
  ("Aylık"/"Yıllık") birleşince ekranda "ZanKurd Pro Aylık" okunur ve
  mağaza kaydındaki "ZanKurd Pro Monthly" ile eşleşir.
- **Yıllık plana birim fiyat eklendi** (build 19). Yıllık kartta yalnız
  "₺399,99/yıl" yazıyordu; artık altında "≈ ₺33,33/ay" durur. Sayı,
  mağazanın kendi fiyat dizesinin biçimi korunarak yazılır — para birimi
  simgesi ve ondalık ayırıcı `storeProduct.priceString`ten alınır.

Uygulama tarafındaki diğer iki koşul zaten karşılanıyordu ve değişmedi:
abonelik süresi (`/ay`, `/yıl` ekleri ve kart başlıkları) ve paywall
üzerindeki çalışan gizlilik + kullanım koşulları bağlantıları
(`LegalLinksRow`). Dört yasal URL de 200 döner: zankurd.com ve
www.zankurd.com altında privacy.html ve terms.html.

Bekçi: `test/subscription_disclosure_test.dart` — abonelik adının satın alma
ekranında iki dilde de yazdığını ve birim fiyat biçimlendirmesinin dört
para birimi düzeninde bozulmadığını ölçer.
