...
List<QuizQuestion>? loadSyncIfInTest() {
  try {
    ...
    final sentenceRaw = File(
      'assets/data/sentence_building_questions.json',
    ).readAsStringSync();
    final offline = (json.decode(offlineRaw) as List).map(
      (e) => QuizQuestion.fromJson(e as Map<String, dynamic>),
    );
    final editorial = (json.decode(editorialRaw) as List).map(
      (e) => QuizQuestion.fromJson(e as Map<String, dynamic>),
    );
    final sentences = (json.decode(sentenceRaw) as List).map(
      (e) => QuizQuestion.fromJson(e as Map<String, dynamic>),
    );
    return [...curatedQuestionBank, ...sentences, ...editorial, ...offline];
  } catch (_) {
    return null;
  }
}
