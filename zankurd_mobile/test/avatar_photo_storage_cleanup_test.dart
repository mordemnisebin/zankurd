...
void main() {
  ...
  test('silme yalnız gerçekten fotoğraf kaldırıldığında yapılıyor', () {
    final src = File(
      'lib/src/screens/avatar_editor_screen.dart',
    ).readAsStringSync();
    expect(
      src,
      contains('_identity.photoUrl == null && _hadPhotoOnOpen'),
      reason:
          'hiç fotoğrafı olmayan kullanıcı yalnız rengini değiştirdiğinde de '
          'gereksiz silme çağrısı gidiyor',
    );
  });
